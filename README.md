...
Your phone is a pocket PC with 8GB+ RAM and an ARM64 CPU — it deserves a real desktop. XLabs sets one up declaratively, in one step:
...
rest — Termux packages, Debian, the `xlabs` launcher. Safe to re-run; each
step skips what's already done, so an interrupted install just resumes.
...
published to `ghcr.io/arinadi/xlabs`) and your **user layer** (whatever you
install inside Debian — survives restarts, wiped by Reset).
...
```mermaid
flowchart LR
    A[Audio] --> B[GPU renderer] --> C[X11] --> D[Debian session]
```
...
Debian half-set-up. A watchdog kills the whole process tree if it hangs:
...
Termux's Style menu), not something XLabs controls from the Python side.

### Start Desktop

1. **Wake lock** — keeps Android from freezing the session
2. **Audio** — PulseAudio, plus the socket Debian will use
...
full diagnostic report is collected automatically.

### Stop Desktop

<!-- screenshot: stop -->

Innermost first: Debian's proot tree, then X11, then audio, then a sweep for
anything left over. No polite XFCE logout — a fresh proot login has no D-Bus
session to ask for one, so it's `TERM` then `KILL` throughout, and it
**verifies with `pgrep`** rather than just claiming success.

### Update

`git pull --ff-only`, hard-resetting to `origin/main` if that's refused. A
**Restart** button relaunches `xlabs` on the new code — pulling alone doesn't
...
A package browser for Debian. Search, see what's already installed (**I**),
tap **Install** — apt runs inside Debian with output streamed live, Termux
untouched. First search is slower: the image ships without package lists to
stay small, so it fetches them once.
...
One screen, full checkup: internet, Python, Debian, storage, DNS, timezone,
GPU, audio, and more — each shown as ● present, ○ missing, or **?** unknown.
...
- **Dupes** — tools installed in both Termux and Debian; can remove the
  Termux copies, Debian treated as primary
...
restart. **Uninstall XLabs** lives here too, behind the same confirm as
Reset.
...
Debian rather than copying files from the host, since proot's ownership and
...
Debian ships Mesa userspace either way, so OpenGL works regardless.
...
By design: proot-distro binds Termux's `$PREFIX` into Debian and appends it
to Debian's `PATH`, so Termux's binaries are reachable inside Debian as a
fallback (Debian's own copy always wins when both exist). `--shared-tmp`
does the same for `/tmp` — it's literally the same directory on both sides.

`Doctor → Dupes` finds tools installed on both and can remove the Termux
copies, on the assumption Debian is where you work. It never touches
anything XLabs itself needs.
...
cgroups for regular apps), not something proot or XLabs can patch around.
Plain rootless Podman hits the same wall even without proot involved at
all, so this isn't an XLabs gap specifically.
...
tool — [Store](#store) installs it straight into Debian. No container
needed for that.
